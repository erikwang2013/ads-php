...
class Meituan implements PlatformAdapter
{
    ...
    protected string $baseUrl = 'https://api.meituan.com/ad/v1/';
    protected string $authBaseUrl = 'https://api.meituan.com/oauth2/';
    ...
    public function name(): string { return '美团广告'; }
    ...
    protected function request(string $method, string $path, array $params = [], ?string $accessToken = null, ?string $overrideBase = null): array
    {
        ...
        if ($errno !== 0) {
            $error = curl_error($ch);
            curl_close($ch);
            throw new RuntimeException('Meituan API network error: ' . $error);
        }
        ...
        if ($httpCode !== 200 || ($decoded['code'] ?? -1) !== 0) {
            throw new RuntimeException(
                'Meituan API error: ' . ($decoded['message'] ?? 'HTTP ' . $httpCode)
            );
        }
        return $decoded;
    }
