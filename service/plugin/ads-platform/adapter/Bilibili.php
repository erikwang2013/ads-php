...
class Bilibili implements PlatformAdapter
{
    ...
    protected string $baseUrl = 'https://api.bilibili.com/x/ad/';
    protected string $authBaseUrl = 'https://api.bilibili.com/x/ad/oauth/';
    ...
    public function name(): string { return 'B站花火'; }
    ...
    public function fetchReports(string $accessToken, string $accountId, ReportRequest $req): Generator
    {
        ...
        $page = 1;
        do {
            $resp = $this->request('GET', 'report/campaign', [
    ...
    public function updateCampaign(string $accessToken, string $accountId, string $platformId, CampaignData $data): void
    {
        ...
        if ($data->dailyBudget > 0) {
            $params['budget'] = $data->dailyBudget;
        }
    ...
    protected function request(string $method, string $path, array $params = [], ?string $accessToken = null, ?string $overrideBase = null): array
    {
        ...
        if ($errno !== 0) {
            $error = curl_error($ch);
            curl_close($ch);
            throw new RuntimeException('Bilibili API network error: ' . $error);
        }
        ...
        if ($httpCode !== 200 || ($decoded['code'] ?? -1) !== 0) {
            throw new RuntimeException(
                'Bilibili API error: ' . ($decoded['message'] ?? 'HTTP ' . $httpCode)
            );
        }
        return $decoded;
    }
