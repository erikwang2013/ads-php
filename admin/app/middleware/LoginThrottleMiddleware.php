...
class LoginThrottleMiddleware implements MiddlewareInterface
{
    ...
    public function process(Request $request, callable $handler): Response
    {
        ...
        $key = 'login_throttle:admin:' . md5($username);
        ...
        try {
            ...
            if ($attempts >= $this->maxAttempts) {
                $ttl = $redis->ttl($key);
                return new Response(429, ['Content-Type' => 'application/json', 'Retry-After' => (string) max(0, $ttl)], json_encode([
                    'code' => 429, 'message' => "Too many login attempts. Retry after {$ttl}s",
