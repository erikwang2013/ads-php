# دعم Flutter عبر منصات سطح المكتب — خطة التنفيذ
...
**الهدف:** توسيع تطبيق Flutter إلى iPadOS وmacOS وWindows وLinux بواجهة لوحة إدارة سطح مكتب كلاسيكية (شريط جانبي بمستويين، شريط عنوان مخصص، مسار تنقل Breadcrumb).

**البنية:** إعادة كتابة طبقة الهيكل (AppShell → DesktopShell، SideNav، TitleBar، BreadcrumbBar) حول تكوين قائمة مشترك. صفحات الميزات التجارية (لوحة التحكم، الحملات، التقارير، الحسابات، التنبيهات) دون تغيير. يغلف GoRouter ShellRoute جميع المسارات في DesktopShell. ملف بيانات واحد `menu_config.dart` يقود كلاً من عرض التنقل وتوليد مسار التنقل.

**Tech Stack:** Flutter 3.2+, Dart, Riverpod, GoRouter, Dio, fl_chart, window_manager ^0.3.0

---

### المهمة 1: إضافة تبعية window_manager وتوليد أدلة منصات سطح المكتب

**Files:**
- Modify: `apps/flutter/pubspec.yaml`
- Generate: `apps/flutter/macos/`, `apps/flutter/windows/`, `apps/flutter/linux/`

- [x] **الخطوة 1: إضافة window_manager إلى pubspec.yaml**

في `apps/flutter/pubspec.yaml`، أضف `window_manager: ^0.3.0` تحت dependencies:
...
- [x] **الخطوة 2: تشغيل flutter pub get**

```bash
cd /home/wwwroot/ads-php/apps/flutter && flutter pub get
```

المتوقع: حل التبعيات دون أخطاء.

- [x] **الخطوة 3: توليد أدلة منصات سطح المكتب**

```bash
cd /home/wwwroot/ads-php/apps/flutter && flutter create --platforms=macos,windows,linux .
```

المتوقع: إنشاء الأدلة `macos/` و`windows/` و`linux/`. يستخدم الأمر `.` لاستهداف المشروع الموجود (بدون استبدال lib/).

- [x] **الخطوة 4: التحقق من وجود أدلة المنصات**
...
المتوقع: طباعة مسارات الأدلة الثلاثة.
...
### المهمة 4: كتابة مكوّن BreadcrumbBar
...
### المهمة 5: إعادة كتابة SideNav بقائمة بمستويين وطيّ
...
### المهمة 6: إعادة كتابة AppShell باسم DesktopShell
...
ملاحظة: يبقى اسم الفئة `AppShell` لتجنب تغيير الاستيراد في `router.dart`. التنفيذ الآن تخطيط سطح مكتب.
...
### المهمة 7: تحديث الموجه بمسارات من مستويين
...
التغيير الرئيسي: `/campaigns` → `/campaigns/list` ليطابق تكوين القائمة ذات المستويين.

- [x] **الخطوة 2: التحقق من ترجمة المشروع كاملاً**
...
### المهمة 8: تهيئة window_manager في main.dart
...
### المهمة 9: ضبط السمة لمظهر سطح المكتب
...
### المهمة 11: تهيئة نوافذ المنصات الأصلية (إخفاء أشرطة العناوين الأصلية)
...
- [x] **الخطوة 1: قراءة ملف macOS Runner للعثور على نقطة التعديل الصحيحة**

```bash
cat /home/wwwroot/ads-php/apps/flutter/macos/Runner/MainFlutterWindow.swift
```

المتوقع: يحتوي على `import Cocoa` وفئة `MainFlutterWindow` مع `awakeFromNib`.

- [x] **الخطوة 2: تهيئة macOS لشريط عنوان مخفي**

أضف بعد `super.awakeFromNib()` في `MainFlutterWindow.swift`:
...
لفعل ذلك، حرّر دالة `awakeFromNib` في `apps/flutter/macos/Runner/MainFlutterWindow.swift` إلى:
...
- [x] **الخطوة 3: تهيئة Windows لشريط عنوان مخفي**
...
ثم حدد موقع استدعاء `CreateWindow` وعدّل معامل `dwStyle`. غيّر `WS_OVERLAPPEDWINDOW` ليشمل النمط بلا حدود، أو أضف منطق `SW_HIDE`. ولأن `window_manager` يتولى هذا في وقت التشغيل، نحتاج للنافذة الأولية تعديل `Win32Window::Create` في `windows/runner/win32_window.cpp`:

```bash
cat /home/wwwroot/ads-php/apps/flutter/windows/runner/win32_window.cpp
```

ابحث عن استدعاء `CreateWindow` وغيّر:
```cpp
...
هذا يزيل شريط العنوان والحدود الأصلية. سيدير `window_manager` بعدها إطارات النوافذ.

- [x] **الخطوة 4: تهيئة Linux لشريط عنوان مخفي**
...
في دالة `my_application_activate`، قبل `gtk_window_present`، أضف:

```cpp
gtk_window_set_decorated(GTK_WINDOW(window), FALSE);
```

- [x] **الخطوة 5: التحقق من سلامة صياغة الملفات الثلاثة (إعادة القراءة)**
...
المتوقع: كل قسم يظهر التعديلات المتوقعة.
...
### المهمة 12: التحقق الشامل من طرف إلى طرف

- [x] **الخطوة 1: تحليل ثابت كامل**
...
- [x] **الخطوة 2: التحقق من ترجمة بناء الويب**
...
- [x] **الخطوة 3: فحص بناء macOS (إن كنا على macOS — تخطَّ إن لم يكن)**
...
- [x] **الخطوة 4: فحص مطابقة قائمة الملفات للمواصفات**
...
المتوقع: جميع الملفات الجديدة موجودة، وtop_bar.dart "لا يوجد مثل هذا الملف".

- [x] **الخطوة 5: الالتزام بالحالة النهائية إذا بقيت أي تغييرات**

```bash
cd /home/wwwroot/ads-php/apps/flutter && git status
```

إذا كانت الشجرة نظيفة، لا حاجة للالتزام. إذا كانت متسخة، أضف إلى المرحلة والتزم:
...
1. اجتياز `dart analyze lib/` بدون أي مشكلات
2. ترجمة `flutter build web` دون أخطاء
3. ترجمة `flutter build macos` (على macOS)
4. وجود جميع الملفات من جدول تغييرات المواصفات في المواقع الصحيحة
5. حذف `top_bar.dart`
6. عدم تعديل ملفات الميزات التجارية (تحقق عبر `git diff main -- lib/features/dashboard/ lib/features/campaign/ lib/features/report/ lib/features/account/ lib/features/alert/ lib/features/auth/`)
