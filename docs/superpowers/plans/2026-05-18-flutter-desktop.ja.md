# Flutter Desktop クロスプラットフォーム対応 — Implementation Plan
...
**Goal:** Flutter アプリを iPadOS、macOS、Windows、Linux に拡張し、クラシックなデスクトップ管理パネル UI（二段階サイドバー、カスタムタイトルバー、パンくず）を実装します。

**Architecture:** 共有メニュー設定を中心にシェル層（AppShell → DesktopShell、SideNav、TitleBar、BreadcrumbBar）を書き換えます。業務機能ページ（dashboard、campaigns、reports、accounts、alerts）は変更しません。GoRouter の ShellRoute がすべてのルートを DesktopShell でラップします。単一の `menu_config.dart` データファイルがナビゲーション描画とパンくず生成の両方を駆動します。

**Tech Stack:** Flutter 3.2+, Dart, Riverpod, GoRouter, Dio, fl_chart, window_manager ^0.3.0

---

### Task 1: window_manager 依存の追加とデスクトッププラットフォームディレクトリの生成

**Files:**
- Modify: `apps/flutter/pubspec.yaml`
- Generate: `apps/flutter/macos/`, `apps/flutter/windows/`, `apps/flutter/linux/`

- [x] **Step 1: pubspec.yaml に window_manager を追加**

`apps/flutter/pubspec.yaml` の dependencies に `window_manager: ^0.3.0` を追加:
...
- [x] **Step 3: デスクトッププラットフォームディレクトリを生成**

```bash
cd /home/wwwroot/ads-php/apps/flutter && flutter create --platforms=macos,windows,linux .
```

期待結果: `macos/`、`windows/`、`linux/` ディレクトリが作成される。このコマンドは `.` を使って既存プロジェクトを対象とします（lib/ は上書きされない）。

- [x] **Step 4: プラットフォームディレクトリの存在を確認**
...
- [x] **Step 1: config ディレクトリを作成**

```bash
mkdir -p /home/wwwroot/ads-php/apps/flutter/lib/config
```

- [x] **Step 2: menu_config.dart を書く**
...
- [x] **Step 3: 構文チェックでファイルがコンパイルされることを確認**
...
### Task 4: BreadcrumbBar ウィジェットの作成

**Files:**
- Create: `apps/flutter/lib/features/shell/breadcrumb.dart`

- [x] **Step 1: breadcrumb.dart を書く**
...
### Task 5: SideNav を二段階メニュー + 折りたたみで書き換え

**Files:**
- Rewrite: `apps/flutter/lib/features/shell/side_nav.dart`

- [x] **Step 1: 新しい side_nav.dart を書く**
...
### Task 6: AppShell を DesktopShell として書き換え

**Files:**
- Rewrite: `apps/flutter/lib/features/shell/app_shell.dart`

- [x] **Step 1: 新しい app_shell.dart（DesktopShell）を書く**
...
注: クラス名は `AppShell` のままにします（`router.dart` の import を変更しないため）。実装はデスクトップレイアウトになっています。
...
### Task 7: 二段階ルートで router を更新

**Files:**
- Modify: `apps/flutter/lib/router.dart`

- [x] **Step 1: router.dart を書き換え**
...
主要な変更: `/campaigns` → `/campaigns/list`（二段階メニュー設定と一致させる）。

- [x] **Step 2: プロジェクト全体がコンパイルされることを確認**
...
### Task 8: main.dart で window_manager を初期化

**Files:**
- Modify: `apps/flutter/lib/main.dart`

- [x] **Step 1: main.dart を書き換え**
...
### Task 9: デスクトップ表示用にテーマを調整

**Files:**
- Modify: `apps/flutter/lib/theme.dart`

- [x] **Step 1: theme.dart を更新**
...
### Task 11: ネイティブプラットフォームウィンドウの設定（ネイティブタイトルバーの非表示）
...
- [x] **Step 1: macOS Runner ファイルを読み、編集ポイントを確認**

```bash
cat /home/wwwroot/ads-php/apps/flutter/macos/Runner/MainFlutterWindow.swift
```

期待結果: `import Cocoa` と、`awakeFromNib` を持つ `MainFlutterWindow` クラスが含まれる。

- [x] **Step 2: macOS をタイトルバー非表示に設定**

`MainFlutterWindow.swift` の `super.awakeFromNib()` の後に追加:
...
これを行うには、`apps/flutter/macos/Runner/MainFlutterWindow.swift` の `awakeFromNib` メソッドを次のように編集します:
...
- [x] **Step 3: Windows をタイトルバー非表示に設定**
...
次に `CreateWindow` 呼び出しを見つけて `dwStyle` パラメータを変更します。`WS_OVERLAPPEDWINDOW` をボーダーレススタイルに変更するか、`SW_HIDE` ロジックを追加します。`window_manager` がこれを実行時に処理するため、初期ウィンドウでは `windows/runner/win32_window.cpp` の `Win32Window::Create` を変更する必要があります:

```bash
cat /home/wwwroot/ads-php/apps/flutter/windows/runner/win32_window.cpp
```

`CreateWindow` 呼び出しを見つけて変更:
```cpp
...
これでネイティブタイトルバーと境界線が削除されます。`window_manager` がウィンドウフレームを管理します。

- [x] **Step 4: Linux をタイトルバー非表示に設定**
...
`my_application_activate` 関数で、`gtk_window_present` の前に追加:

```cpp
gtk_window_set_decorated(GTK_WINDOW(window), FALSE);
```

- [x] **Step 5: 3 ファイルすべてが構文的に無傷であることを確認（読み戻し）**
...
- [x] **Step 2: Web ビルドがコンパイルされることを確認**
...
- [x] **Step 3: macOS ビルドを確認（macOS の場合のみ — それ以外はスキップ）**
...
期待結果: すべての新規ファイルが存在し、top_bar.dart は "No such file"。

- [x] **Step 5: 残変更があれば最終状態をコミット**

```bash
cd /home/wwwroot/ads-php/apps/flutter && git status
```

クリーンならコミット不要。ダーティならステージしてコミット:
...
1. `dart analyze lib/` がゼロ問題で通過
2. `flutter build web` がエラーなくコンパイル
3. `flutter build macos` がコンパイル（macOS 上）
4. 仕様の変更表の全ファイルが正しい場所に存在
5. `top_bar.dart` が削除されている
6. 業務機能ファイルが変更されていない（`git diff main -- lib/features/dashboard/ lib/features/campaign/ lib/features/report/ lib/features/account/ lib/features/alert/ lib/features/auth/` で検証）
