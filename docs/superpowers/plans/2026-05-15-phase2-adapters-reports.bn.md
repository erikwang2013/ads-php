...
**Goal:** নতুন 百度营销 এবং 淘宝广告 অ্যাডাপ্টার যোগ করা, প্রকৃত ডেটা সিঙ্ক শিডিউলিং বাস্তবায়ন, কাস্টম মাল্টি-ডাইমেনশন কোয়েরি সাপোর্টের জন্য রিপোর্ট ইঞ্জিন এনহান্স করা।

**Architecture:** প্রতিটি অ্যাড প্ল্যাটফর্ম PlatformAdapter ইন্টারফেস বাস্তবায়ন করে, AdapterRegistry-তে ইউনিফাইডভাবে রেজিস্টার করা হয়। ডেটা সিঙ্ক webman/crontab টাইমড শিডিউলিং ব্যবহার করে, সিঙ্ক প্রক্রিয়ায় প্ল্যাটফর্মের নেটিভ ডেটা FieldMapping দিয়ে ইউনিফাইড ফরম্যাটে রূপান্তর করে report_metrics টেবিলে লেখা হয়। রিপোর্ট ইঞ্জিন বিদ্যমান DashboardController-এর ভিত্তিতে CampaignController-এ কাস্টম কোয়েরি ক্ষমতা যোগ করে।

**Tech Stack:** PHP 8.2+, webman v2, webman/crontab, curl (API calls), Vue 3, ECharts 5

**Phase 1-এর বিদ্যমান ভিত্তি:**
- PlatformAdapter ইন্টারফেস、AdapterRegistry、FieldMapping সব প্রস্তুত
- 巨量引擎 অ্যাডাপ্টার সম্পূর্ণ চেইন বাস্তবায়িত (OAuth → সিঙ্ক → ডেলিভারি অপারেশন)
- report_metrics টেবিল তৈরি, DashboardController::summary()-এ তারিখ/প্ল্যাটফর্ম অ্যাগ্রিগেশন ক্ষমতা আছে
- campaigns টেবিল তৈরি, CampaignController-এ সম্পূর্ণ CRUD আছে
- OAuthService জেনেরিক অথোরাইজেশন ফ্লো প্রস্তুত

---

## ফাইল স্ট্রাকচার (Phase 2 নতুন/পরিবর্তিত)
...
### Task 10: 百度营销 অ্যাডাপ্টার তৈরি

**Files:**
- Create: `service/plugin/ads-platform/adapter/Baidu.php`
- Modify: `service/plugin/ads-platform/config/bootstrap.php`

百度营销 API কোর ইন্টিগ্রেশন:
- OAuth অথোরাইজেশন অ্যাড্রেস: `https://u.baidu.com/oauth/authorize`
- Token এক্সচেঞ্জ: `https://u.baidu.com/oauth/token`
- অ্যাড প্ল্যান কোয়েরি: `/json/sms/service/CampaignService/getCampaign`
- রিপোর্ট কোয়েরি: `/json/sms/service/ReportService/getProfessionalReportId` + `getReportData`

#### Step 1: Baidu.php অ্যাডাপ্টার তৈরি
...
#### Step 2: 百度 অ্যাডাপ্টার রেজিস্টার করুন
...
### Task 11: 淘宝广告 অ্যাডাপ্টার তৈরি

**Files:**
- Create: `service/plugin/ads-platform/adapter/Taobao.php`
- Modify: `service/plugin/ads-platform/config/bootstrap.php`

淘宝广告 (阿里妈妈) API কোর ইন্টিগ্রেশন:
- OAuth অথোরাইজেশন: 淘宝开放平台 OAuth 2.0
- API এন্ডপয়েন্ট: `https://api.taobao.com/router/rest`
- অ্যাড প্ল্যান: `alimama.campaign.get`
- রিপোর্ট: `alimama.report.get`

#### Step 1: Taobao.php তৈরি
...
#### Step 2: 淘宝 অ্যাডাপ্টার রেজিস্টার করুন
...
### Task 12: ডেটা সিঙ্ক টাস্ক + রিপোর্ট ইঞ্জিন এনহান্সমেন্ট তৈরি
...
#### Step 4: cron.php টাইমড টাস্ক কনফিগ তৈরি
...
#### Step 5: ads-report প্লাগইন + ReportBuilder তৈরি
...
#### Step 7: রিপোর্ট রুট যোগ

`route.php`-এর অথেনটিকেশন গ্রুপে যোগ করুন:
```php
...
1. ✅ 百度营销 অ্যাডাপ্টার PlatformAdapter-এর সব মেথড বাস্তবায়ন করেছে
2. ✅ 淘宝广告 অ্যাডাপ্টার PlatformAdapter-এর সব মেথড বাস্তবায়ন করেছে (淘宝 সাইনিং অ্যালগরিদমসহ)
3. ✅ OAuth কলব্যাক প্রসেসিং AdapterRegistry-এর মাধ্যমে ইউনিফাইড分流
4. ✅ 百度/淘宝 অ্যাডাপ্টার bootstrap.php-এ রেজিস্টার করা হয়েছে
5. ✅ TokenRefreshTask প্রতি ৫৫ মিনিটে মেয়াদোত্তীর্ণ Token স্ক্যান ও রিফ্রেশ করে
6. ✅ DataSyncTask প্রতি ১০ মিনিটে প্রতিটি প্ল্যাটফর্মের ডেলিভারি ডেটা এবং রিপোর্ট পুল করে
7. ✅ রিপোর্ট কাস্টম কোয়েরি API মাল্টি-ডাইমেনশন (platform/date/campaign) এবং মাল্টি-মেট্রিক কম্বিনেশন সাপোর্ট করে
8. ✅ সিঙ্ক করা ডেটা সঠিকভাবে report_metrics টেবিলে লেখা হয়
