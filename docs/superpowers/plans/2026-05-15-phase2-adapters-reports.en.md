# Phase 2: Ad Platform Expansion + Report Engine Implementation Plan
...
**Goal:** Add Baidu marketing and Taobao ads adapters, implement real data-sync scheduling, and enhance the report engine to support custom multi-dimensional queries.

**Architecture:** Each ad platform implements the PlatformAdapter interface, registered uniformly via AdapterRegistry. Data sync is scheduled via webman/crontab; during sync, native platform data is converted to the unified format via FieldMapping and written to the report_metrics table. The report engine adds custom query capability via the new ReportController, on top of the existing DashboardController.

**Tech Stack:** PHP 8.2+, webman v2, webman/crontab, curl (API calls), Vue 3, ECharts 5

**Phase 1 Existing Foundation:**
- PlatformAdapter interface, AdapterRegistry, FieldMapping all ready
- Juliang engine adapter implements the full pipeline (OAuth → sync → campaign operations)
- report_metrics table is ready; DashboardController::summary() already aggregates by date/platform
- campaigns table is ready; CampaignController has full CRUD
- OAuthService generic authorization flow is ready
...
### Task 10: Create the Baidu marketing adapter

**Files:**
- Create: `service/plugin/ads-platform/adapter/Baidu.php`
- Modify: `service/plugin/ads-platform/config/bootstrap.php`

Baidu marketing API core integration:
- OAuth authorization URL: `https://u.baidu.com/oauth/authorize`
- Token exchange: `https://u.baidu.com/oauth/token`
- Campaign query: `/json/sms/service/CampaignService/getCampaign`
- Report query: `/json/sms/service/ReportService/getProfessionalReportId` + `getReportData`

#### Step 1: Create the Baidu.php adapter
...
#### Step 2: Register the Baidu adapter
...
### Task 11: Create the Taobao ads adapter

**Files:**
- Create: `service/plugin/ads-platform/adapter/Taobao.php`
- Modify: `service/plugin/ads-platform/config/bootstrap.php`

Taobao ads (Alimama) API core integration:
- OAuth: Taobao Open Platform OAuth 2.0
- API endpoint: `https://api.taobao.com/router/rest`
- Campaigns: `alimama.campaign.get`
- Reports: `alimama.report.get`

#### Step 1: Create Taobao.php
...
#### Step 2: Register the Taobao adapter
...
### Task 12: Create data sync tasks + report engine enhancements
...
#### Step 1: Create the ads-task plugin
...
#### Step 2: Create TokenRefreshTask
...
#### Step 4: Create the cron.php scheduled task config
...
#### Step 5: Create the ads-report plugin + ReportBuilder
...
#### Step 7: Add the report route

Add in the `route.php` auth group:
```php
...
1. ✅ Baidu marketing adapter implements all PlatformAdapter methods
2. ✅ Taobao ads adapter implements all PlatformAdapter methods (including the Taobao signing algorithm)
3. ✅ OAuth callback handling is routed uniformly through AdapterRegistry
4. ✅ Baidu/Taobao adapters are registered in bootstrap.php
5. ✅ TokenRefreshTask scans for expired tokens every 55 minutes and refreshes them
6. ✅ DataSyncTask pulls campaign data and reports from each platform every 10 minutes
7. ✅ Custom report query API supports multi-dimension (platform/date/campaign) and multi-metric combinations
8. ✅ Synced data is correctly written to the report_metrics table
