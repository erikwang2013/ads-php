# Phase 2: Plano de implementação de expansão de plataformas de anúncios + engine de relatórios
...
**Objetivo:** adicionar os adaptadores de Baidu Marketing e Taobao Ads, implementar o agendamento real de sincronização de dados e aprimorar o engine de relatórios para suportar consultas personalizadas de múltiplas dimensões.

**Arquitetura:** cada plataforma de anúncios implementa a interface PlatformAdapter, registrada de forma unificada via AdapterRegistry. A sincronização de dados usa o agendamento webman/crontab; durante a sincronização, os dados nativos da plataforma são convertidos para o formato unificado via FieldMapping e gravados na tabela report_metrics. O engine de relatórios adiciona a capacidade de consulta personalizada no CampaignController, além do DashboardController existente.

**Tech Stack:** PHP 8.2+, webman v2, webman/crontab, curl (API calls), Vue 3, ECharts 5

**Base existente do Phase 1:**
- Interface PlatformAdapter, AdapterRegistry e FieldMapping totalmente prontos
- O adaptador da Juliang já implementa o caminho completo (OAuth → sincronização → operações de veiculação)
- A tabela report_metrics criada, e DashboardController::summary() já tem capacidade de agregação por data/plataforma
- A tabela campaigns criada, e CampaignController já tem CRUD completo
- O fluxo de autorização genérico do OAuthService pronto

---

## Estrutura de arquivos (novos/modificados no Phase 2)
...
### Task 10: criar o adaptador de Baidu Marketing

**Files:**
- Create: `service/plugin/ads-platform/adapter/Baidu.php`
- Modify: `service/plugin/ads-platform/config/bootstrap.php`

Integração central com a API do Baidu Marketing:
- URL de autorização OAuth: `https://u.baidu.com/oauth/authorize`
- Troca de Token: `https://u.baidu.com/oauth/token`
- Consulta de planos de anúncios: `/json/sms/service/CampaignService/getCampaign`
- Consulta de relatórios: `/json/sms/service/ReportService/getProfessionalReportId` + `getReportData`

#### Step 1: criar o adaptador Baidu.php
...
#### Step 2: registrar o adaptador Baidu
...
### Task 11: criar o adaptador de Taobao Ads

**Files:**
- Create: `service/plugin/ads-platform/adapter/Taobao.php`
- Modify: `service/plugin/ads-platform/config/bootstrap.php`

Integração central com a API do Taobao Ads (Alimama):
- Autorização OAuth: OAuth 2.0 da plataforma aberta do Taobao
- Endpoint da API: `https://api.taobao.com/router/rest`
- Planos de anúncios: `alimama.campaign.get`
- Relatórios: `alimama.report.get`

#### Step 1: criar Taobao.php
...
#### Step 2: registrar o adaptador Taobao
...
### Task 12: criar a tarefa de sincronização de dados + aprimorar o engine de relatórios
...
#### Step 1: criar o plugin ads-task
...
#### Step 4: criar a configuração de tarefas agendadas cron.php
...
#### Step 5: criar o plugin ads-report + ReportBuilder
...
#### Step 7: adicionar as rotas de relatórios

Adicionar no grupo autenticado de `route.php`:
```php
...
1. ✅ O adaptador de Baidu Marketing implementa todos os métodos do PlatformAdapter
2. ✅ O adaptador de Taobao Ads implementa todos os métodos do PlatformAdapter (incluindo o algoritmo de assinatura do Taobao)
3. ✅ O tratamento do callback OAuth é distribuído de forma unificada via AdapterRegistry
4. ✅ Os adaptadores Baidu/Taobao registrados em bootstrap.php
5. ✅ TokenRefreshTask escaneia Tokens expirados a cada 55 minutos e os atualiza
6. ✅ DataSyncTask busca os dados de veiculação e relatórios de cada plataforma a cada 10 minutos
7. ✅ A API de consulta personalizada de relatórios suporta múltiplas dimensões (platform/date/campaign) e combinações de múltiplas métricas
8. ✅ Os dados sincronizados são gravados corretamente na tabela report_metrics
