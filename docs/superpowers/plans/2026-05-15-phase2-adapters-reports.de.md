# Phase 2: Implementierungsplan Plattform-Erweiterung + Report-Engine
...
> **Für agentic workers:** ERFORDERLICHE SUB-SKILL: superpowers:subagent-driven-development zur taskweisen Implementierung dieses Plans verwenden.

**Ziel:** Baidu-Marketing- und Taobao-Ad-Adapter neu hinzufügen, echte Datensynchronisierungs-Planung implementieren und die Report-Engine um benutzerdefinierte multidimensionale Abfragen erweitern.

**Architektur:** Jede Werbeplattform implementiert das PlatformAdapter-Interface und wird zentral über die AdapterRegistry registriert. Die Datensynchronisierung nutzt webman/crontab zur Zeitplanung; während der Synchronisierung werden die nativen Plattformdaten über FieldMapping in das einheitliche Format überführt und in die report_metrics-Tabelle geschrieben. Die Report-Engine erweitert die benutzerdefinierten Abfragefähigkeiten im CampaignController auf Basis des bestehenden DashboardController.

**Tech Stack:** PHP 8.2+, webman v2, webman/crontab, curl (API-Aufrufe), Vue 3, ECharts 5

**Basis aus Phase 1:**
- PlatformAdapter-Interface, AdapterRegistry, FieldMapping alle bereit
- Juliang-Adapter mit vollständiger Kette implementiert (OAuth → Synchronisierung → Werbeoperationen)
- report_metrics-Tabelle erstellt, DashboardController::summary() mit Aggregation nach Datum/Plattform
- campaigns-Tabelle erstellt, CampaignController mit vollständigem CRUD
- OAuthService mit allgemeinem Autorisierungsablauf bereit

---

## Dateistruktur (Phase 2 neu/geändert)
...
### Task 10: Baidu-Marketing-Adapter erstellen

**Dateien:**
- Erstellen: `service/plugin/ads-platform/adapter/Baidu.php`
- Ändern: `service/plugin/ads-platform/config/bootstrap.php`

Kernanbindung der Baidu-Marketing-API:
- OAuth-Autorisierungsadresse: `https://u.baidu.com/oauth/authorize`
- Token-Austausch: `https://u.baidu.com/oauth/token`
- Werbeplan-Abfrage: `/json/sms/service/CampaignService/getCampaign`
- Berichtsabfrage: `/json/sms/service/ReportService/getProfessionalReportId` + `getReportData`

#### Schritt 1: Baidu.php-Adapter erstellen
...
#### Schritt 2: Baidu-Adapter registrieren
...
### Task 11: Taobao-Ad-Adapter erstellen

**Dateien:**
- Erstellen: `service/plugin/ads-platform/adapter/Taobao.php`
- Ändern: `service/plugin/ads-platform/config/bootstrap.php`

Kernanbindung der Taobao-Ad-API (阿里妈妈):
- OAuth-Autorisierung: Taobao Open Platform OAuth 2.0
- API-Endpunkt: `https://api.taobao.com/router/rest`
- Werbepläne: `alimama.campaign.get`
- Berichte: `alimama.report.get`

#### Schritt 1: Taobao.php erstellen
...
#### Schritt 2: Taobao-Adapter registrieren
...
### Task 12: Datensynchronisierungsaufgaben + Report-Engine-Erweiterung erstellen

**Dateien:**
- Erstellen: `service/plugin/ads-task/config/plugin.php`
- Erstellen: `service/plugin/ads-task/task/DataSyncTask.php`
- Erstellen: `service/plugin/ads-task/task/TokenRefreshTask.php`
- Erstellen: `service/plugin/ads-task/config/cron.php`
- Erstellen: `service/plugin/ads-report/config/plugin.php`
- Erstellen: `service/plugin/ads-report/service/ReportBuilder.php`
- Erstellen: `service/plugin/ads-api/controller/ReportController.php`
- Ändern: `service/plugin/ads-api/config/route.php`

#### Schritt 1: ads-task-Plugin erstellen
...
#### Schritt 2: TokenRefreshTask erstellen
...
#### Schritt 3: DataSyncTask erstellen
...
#### Schritt 4: cron.php-Konfiguration der geplanten Aufgaben erstellen
...
#### Schritt 5: ads-report-Plugin + ReportBuilder erstellen
...
#### Schritt 6: ReportController erstellen
...
#### Schritt 7: Berichtsroute hinzufügen

In der Authentifizierungsgruppe von `route.php` hinzufügen:
```php
...
1. ✅ Baidu-Marketing-Adapter implementiert alle PlatformAdapter-Methoden
2. ✅ Taobao-Ad-Adapter implementiert alle PlatformAdapter-Methoden (inkl. Taobao-Signaturalgorithmus)
3. ✅ OAuth-Callback-Behandlung einheitlich über die AdapterRegistry verteilt
4. ✅ Baidu-/Taobao-Adapter in bootstrap.php registriert
5. ✅ TokenRefreshTask scannt alle 55 Minuten abgelaufene Token und erneuert sie
6. ✅ DataSyncTask ruft alle 10 Minuten Werbedaten und Berichte aller Plattformen ab
7. ✅ API für benutzerdefinierte Berichtsabfragen unterstützt multidimensionale (platform/date/campaign) und Multi-Metrik-Kombinationen
8. ✅ Synchronisierte Daten werden korrekt in die report_metrics-Tabelle geschrieben
