# Phase 2 : Extension des plateformes publicitaires + Moteur de rapports — Plan d'implémentation
...
> **Pour les agents autonomes :** SOUS-COMPÉTENCE REQUISE : utiliser superpowers:subagent-driven-development pour implémenter ce plan tâche par tâche.

**Objectif :** Ajouter les adaptateurs 百度营销 (Baidu Marketing) et 淘宝广告 (Taobao Ads), implémenter une véritable planification de la synchronisation des données, et renforcer le moteur de rapports pour prendre en charge les requêtes personnalisées multi-dimensionnelles.

**Architecture :** Chaque plateforme publicitaire implémente l'interface PlatformAdapter, enregistrée de manière uniforme via AdapterRegistry. La synchronisation des données utilise la planification webman/crontab ; pendant la synchronisation, les données natives de la plateforme sont converties via FieldMapping au format unifié puis écrites dans la table report_metrics. Le moteur de rapports s'appuie sur le DashboardController existant et ajoute la capacité de requête personnalisée dans CampaignController.

**Tech Stack:** PHP 8.2+, webman v2, webman/crontab, curl (API calls), Vue 3, ECharts 5

**Base existante de la Phase 1 :**
- Interface PlatformAdapter, AdapterRegistry, FieldMapping tous prêts
- L'adaptateur 巨量引擎 implémente la chaîne complète (OAuth → synchronisation → opérations de diffusion)
- Table report_metrics créée, DashboardController::summary() dispose déjà de l'agrégation par date/plateforme
- Table campaigns créée, CampaignController dispose déjà d'un CRUD complet
- Flux d'autorisation générique OAuthService prêt

---

## Structure de fichiers (nouveaux/modifiés en Phase 2)
...
### Task 10: Créer l'adaptateur 百度营销

**Files:**
- Create: `service/plugin/ads-platform/adapter/Baidu.php`
- Modify: `service/plugin/ads-platform/config/bootstrap.php`

Intégration API 百度营销 :
- URL d'autorisation OAuth : `https://u.baidu.com/oauth/authorize`
- Échange de token : `https://u.baidu.com/oauth/token`
- Interrogation des campagnes : `/json/sms/service/CampaignService/getCampaign`
- Interrogation des rapports : `/json/sms/service/ReportService/getProfessionalReportId` + `getReportData`

#### Step 1: Créer l'adaptateur Baidu.php
...
#### Step 2: Enregistrer l'adaptateur Baidu
...
### Task 11: Créer l'adaptateur 淘宝广告

**Files:**
- Create: `service/plugin/ads-platform/adapter/Taobao.php`
- Modify: `service/plugin/ads-platform/config/bootstrap.php`

Intégration API 淘宝广告 (阿里妈妈) :
- Autorisation OAuth : plateforme ouverte Taobao OAuth 2.0
- Point de terminaison API : `https://api.taobao.com/router/rest`
- Campagnes : `alimama.campaign.get`
- Rapports : `alimama.report.get`

#### Step 1: Créer Taobao.php
...
#### Step 2: Enregistrer l'adaptateur Taobao
...
### Task 12: Créer les tâches de synchronisation des données + renforcer le moteur de rapports
...
#### Step 1: Créer le plugin ads-task
...
#### Step 4: Créer la configuration des tâches planifiées cron.php
...
#### Step 5: Créer le plugin ads-report + ReportBuilder
...
#### Step 7: Ajouter la route de rapports

Ajouter dans le groupe authentifié de `route.php` :
```php
...
1. ✅ L'adaptateur 百度营销 implémente toutes les méthodes PlatformAdapter
2. ✅ L'adaptateur 淘宝广告 implémente toutes les méthodes PlatformAdapter (y compris l'algorithme de signature Taobao)
3. ✅ Le traitement du callback OAuth passe par une répartition unifiée via AdapterRegistry
4. ✅ Les adaptateurs Baidu/Taobao sont enregistrés dans bootstrap.php
5. ✅ TokenRefreshTask scanne et rafraîchit les tokens expirés toutes les 55 minutes
6. ✅ DataSyncTask récupère les données de diffusion et les rapports de chaque plateforme toutes les 10 minutes
7. ✅ L'API de requête de rapports personnalisée prend en charge les combinaisons multi-dimensions (platform/date/campaign) et multi-métriques
8. ✅ Les données synchronisées sont correctement écrites dans la table report_metrics
