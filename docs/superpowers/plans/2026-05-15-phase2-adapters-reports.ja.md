# Phase 2: 广告平台拡張 + レポートエンジン Implementation Plan
...
**Goal:** 百度营销と淘宝广告のアダプターを新規追加し、本格的なデータ同期スケジューリングを実装、レポートエンジンを強化してカスタム多次元クエリをサポートします。

**Architecture:** 各広告プラットフォームは PlatformAdapter インターフェースを実装し、AdapterRegistry で統一的に登録します。データ同期は webman/crontab の定期スケジューリングを使用し、同期過程でプラットフォームネイティブデータを FieldMapping で統一形式に変換して report_metrics テーブルに書き込みます。レポートエンジンは既存の DashboardController を基盤に、CampaignController にカスタムクエリ機能を追加します。

**Tech Stack:** PHP 8.2+, webman v2, webman/crontab, curl (API calls), Vue 3, ECharts 5

**Phase 1 の既存基盤:**
- PlatformAdapter インターフェース、AdapterRegistry、FieldMapping はすべて準備完了
- 巨量引擎アダプターは完全なチェーン（OAuth → 同期 → 配信操作）を実装済み
- report_metrics テーブルは作成済み、DashboardController::summary() は日付/プラットフォーム集計能力あり
- campaigns テーブルは作成済み、CampaignController は完全な CRUD あり
- OAuthService の汎用認証フローは準備完了

---

## ファイル構造（Phase 2 新規/変更）
...
│   ├── ads-platform/
│   │   └── adapter/
│   │       ├── Baidu.php              # 新規：百度营销アダプター
│   │       ├── Taobao.php             # 新規：淘宝广告アダプター
│   │       └── Juliang.php            # 変更：API レスポンス例のコメントを補充
│   ├── ads-task/
│   │   ├── config/plugin.php          # 新規
│   │   ├── task/
│   │   │   ├── DataSyncTask.php       # 新規：汎用データ同期タスク
│   │   │   └── TokenRefreshTask.php   # 新規：Token 自動更新タスク
│   │   └── config/cron.php            # 新規：定期タスク設定
│   ├── ads-api/
│   │   ├── controller/
│   │   │   ├── ReportController.php   # 新規：カスタムレポート API
│   │   │   └── CampaignController.php # 変更：バッチ操作 API を追加
│   │   └── config/route.php           # 変更：レポートルートを追加
│   └── ads-report/
│       ├── config/plugin.php          # 新規
│       └── service/
│           └── ReportBuilder.php      # 新規：レポート事前計算サービス
```

---

### Task 10: 百度营销アダプターの作成
...
百度营销 API の核心連携：
- OAuth 認可アドレス：`https://u.baidu.com/oauth/authorize`
- Token 交換：`https://u.baidu.com/oauth/token`
- 広告計画照会：`/json/sms/service/CampaignService/getCampaign`
- レポート照会：`/json/sms/service/ReportService/getProfessionalReportId` + `getReportData`

#### Step 1: Baidu.php アダプターの作成
...
淘宝广告（阿里妈妈）API の核心連携：
- OAuth 認可：淘宝オープンプラットフォーム OAuth 2.0
- API エンドポイント：`https://api.taobao.com/router/rest`
- 広告計画：`alimama.campaign.get`
- レポート：`alimama.report.get`

#### Step 1: Taobao.php の作成
...
### Task 12: データ同期タスク + レポートエンジン強化の作成
...
#### Step 5: ads-report プラグイン + ReportBuilder の作成
...
1. ✅ 百度营销アダプターがすべての PlatformAdapter メソッドを実装
2. ✅ 淘宝广告アダプターがすべての PlatformAdapter メソッドを実装（淘宝署名アルゴリズム含む）
3. ✅ OAuth コールバック処理が AdapterRegistry 経由で統一分流
4. ✅ 百度/淘宝アダプターが bootstrap.php に登録済み
5. ✅ TokenRefreshTask が 55 分ごとに期限切れ Token をスキャンして更新
6. ✅ DataSyncTask が 10 分ごとに各プラットフォームの配信データとレポートを取得
7. ✅ レポートカスタムクエリ API が多次元（platform/date/campaign）と多指標の組み合わせをサポート
8. ✅ 同期データが report_metrics テーブルに正しく書き込まれる
