...
|---|---|
| Визуализация статуса синхронизации | Таблица `ads_sync_errors` + `RetrySyncTask` (3 повтора, экспоненциальная задержка 5^n минут) уже есть; **нет фронтенд-страницы/API для отображения частоты сбоев и задержек синхронизации** |
| Замкнутый цикл данных о конверсиях | Таблицы `ads_conversions` + `ads_attribution_results` уже есть, движок атрибуции реализован; **нет точки входа для сбора данных о конверсиях** (API ретрансляции/трекинга) |
| CI для мобильных платформ | `ci.yml` только PHP-синтаксис→PHPUnit→vue-tsc→Docker; **нет сборки/пакетирования Flutter/HarmonyOS** |
| Мультитенантный SaaS | Таблица `ads_tenants` + middleware TenantIdentify уже есть; **нет биллинга/квот/статистики использования** |
...
- Источники данных: ads_platform_accounts (last_sync_at) + ads_sync_errors
...
- Валидация: существование campaign_id, неотрицательная сумма, формат времени; запись в ads_conversions
...
### Ключевые моменты дизайна
- Данные: добавить поле quota в ads_tenants или новая таблица ads_tenant_quotas (plan/account_limit/campaign_limit/sync_quota)
