...
**목표:** webman v2 서버 + webman-admin v2 관리 백엔드 골격을 구축하고, 멀티 테넌트, 인증 권한, 계정 관리, OAuth 흐름 프레임워크를 구현하며, 巨量引擎(줄량 엔진)을 첫 번째 플랫폼으로 전체 체인을 가동합니다.

**아키텍처:** webman v2 플러그인 아키텍처로, service/plugin/ 아래에 책임별로 ads-tenant, ads-account, ads-platform, ads-api 4개 플러그인을 분리합니다; 관리 백엔드는 Vue3+TS+Element Plus를 사용하고 Pinia로 상태를 관리하며 Axios로 백엔드 API를 연동합니다.
...
### Task 1: webman v2 서버 프로젝트 초기화

**Files:**
- 생성: `service/composer.json`
- 생성: `service/.env`
- 생성: `service/config/app.php`
- 생성: `service/config/database.php`
- 생성: `service/config/redis.php`
- 생성: `service/config/middleware.php`
- 생성: `service/support/Response.php`
- 생성: `service/start.php`

- [x] **Step 1: composer.json 생성 및 의존성 설치**
...
실행: `cd service && composer install`

- [x] **Step 2: .env 환경 설정 생성**
...
- [x] **Step 4: 동적 데이터 소스를 지원하는 config/database.php 생성**
...
- [x] **Step 6: config/middleware.php 생성**
...
- [x] **Step 7: 통일 응답 support/Response.php 생성**
...
실행: `cd service && php start.php start`
예상: webman v2가 성공적으로 시작되고 8787 포트를 리슨
...
### Task 2: ads-tenant 멀티 테넌트 플러그인 생성

**Files:**
- 생성: `service/plugin/ads-tenant/config/plugin.php`
- 생성: `service/plugin/ads-tenant/config/database.php`
- 생성: `service/plugin/ads-tenant/model/Tenant.php`
- 생성: `service/plugin/ads-tenant/middleware/TenantIdentify.php`
- 생성: `service/plugin/ads-tenant/migration/create_tenants.sql`

- [x] **Step 1: plugin.php 생성**
...
- [x] **Step 2: database.php 데이터 소스 라우팅 생성**
...
- [x] **Step 4: TenantIdentify 미들웨어 생성**
...
### Task 3: ads-platform 어댑터 코어 생성

**Files:**
- 생성: `service/plugin/ads-platform/config/plugin.php`
- 생성: `service/plugin/ads-platform/src/PlatformAdapter.php`
- 생성: `service/plugin/ads-platform/src/AdapterRegistry.php`
- 생성: `service/plugin/ads-platform/src/FieldMapping.php`
- 생성: `service/plugin/ads-platform/src/CampaignData.php`
- 생성: `service/plugin/ads-platform/src/ReportRequest.php`
- 생성: `service/plugin/ads-platform/adapter/Juliang.php`

- [x] **Step 1: PlatformAdapter 인터페이스 생성**
...
- [x] **Step 4: FieldMapping 필드 매핑 엔진 생성**
...
- [x] **Step 6: 巨量引擎 어댑터 생성(필드 매핑 + 인터페이스 골격)**
...
- [x] **Step 8: 마이그레이션 SQL 생성(campaigns, ad_groups, creatives, report_metrics, report_extras)**
...
- [x] **Step 9: AdapterRegistry에 巨量引擎 어댑터 등록**

`service/plugin/ads-platform/config/bootstrap.php` 생성:
...
### Task 4: ads-account 계정 관리 플러그인 생성

**Files:**
- 생성: `service/plugin/ads-account/config/plugin.php`
- 생성: `service/plugin/ads-account/model/PlatformAccount.php`
- 생성: `service/plugin/ads-account/model/AuthToken.php`
- 생성: `service/plugin/ads-account/service/OAuthService.php`
- 생성: `service/plugin/ads-account/migration/create_platform_accounts.sql`
...
- [x] **Step 2: PlatformAccount 모델 생성**
...
- [x] **Step 3: AuthToken 모델 생성(OAuth 흐름의 state 저장용)**
...
### Task 5: ads-api 플러그인 생성(RESTful API + JWT 인증)

**Files:**
- 생성: `service/plugin/ads-api/config/plugin.php`
- 생성: `service/plugin/ads-api/config/route.php`
- 생성: `service/plugin/ads-api/middleware/AuthMiddleware.php`
- 생성: `service/plugin/ads-api/controller/AuthController.php`
- 생성: `service/plugin/ads-api/controller/PlatformController.php`
- 생성: `service/plugin/ads-api/controller/AccountController.php`
- 생성: `service/plugin/ads-api/controller/CampaignController.php`
- 생성: `service/plugin/ads-api/controller/DashboardController.php`
...
- [x] **Step 2: JWT AuthMiddleware 생성**
...
- [x] **Step 4: PlatformController 생성**
...
- [x] **Step 6: CampaignController 생성**
...
- [x] **Step 7: DashboardController 생성**
...
### Task 6: webman-admin v2 관리 백엔드 프로젝트 초기화

**Files:**
- 생성: `admin/package.json`
- 생성: `admin/vite.config.ts`
- 생성: `admin/tsconfig.json`
- 생성: `admin/index.html`
- 생성: `admin/src/main.ts`
- 생성: `admin/src/App.vue`
- 생성: `admin/src/router/index.ts`
- 생성: `admin/src/api/index.ts`

- [x] **Step 1: package.json 생성**
...
- [x] **Step 7: src/router/index.ts 생성**
...
- [x] **Step 8: src/api/index.ts 생성(Axios 래핑)**
...
- [x] **Step 9: 의존성 설치 및 검증**

실행: `cd admin && npm install && npm run dev`
예상: Vite 개발 서버가 5173 포트에서 시작
...
### Task 7: 관리 백엔드 레이아웃 & 로그인 페이지

**Files:**
- 생성: `admin/src/components/layout/AppLayout.vue`
- 생성: `admin/src/components/layout/SideNav.vue`
- 생성: `admin/src/components/layout/TopBar.vue`
- 생성: `admin/src/views/login/LoginPage.vue`
- 생성: `admin/src/stores/auth.ts`
- 생성: `admin/src/api/auth.ts`

- [x] **Step 1: auth store 생성**
...
### Task 8: 관리 백엔드 핵심 페이지

**Files:**
- 생성: `admin/src/views/dashboard/DashboardPage.vue`
- 생성: `admin/src/views/account/AccountList.vue`
- 생성: `admin/src/views/account/AccountBind.vue`
- 생성: `admin/src/views/campaign/CampaignList.vue`
- 생성: `admin/src/components/MetricCard.vue`
- 생성: `admin/src/components/PlatformBadge.vue`
- 생성: `admin/src/api/dashboard.ts`
- 생성: `admin/src/api/account.ts`
- 생성: `admin/src/api/campaign.ts`
- 생성: `admin/src/utils/format.ts`

- [x] **Step 1: 유틸 함수 format.ts 생성**
...
- [x] **Step 9: 관리 백엔드 기능 검증**

실행: `cd admin && npm run dev`
설정: vite proxy가 webman 서버 8787 포트를 가리키는지 확인
확인: 로그인 → 대시보드 → 계정 관리 → 계획 생성 → 계획 시작/중지
...
### Task 9: 설정 가이드 & 프로젝트 시작 검증

**Files:**
- 생성: `service/start.php`(있으면 수정)
- 생성: `README.md`

- [x] **Step 1: service/start.php에 플러그인 로드와 데이터베이스 초기화 추가**
...
- [x] **Step 3: 서버 시작 및 API 검증**

실행: `cd service && php start.php start -d`
...
```
예상: `{"code":0,"data":{"access_token":"...","user":{...}}}`

플랫폼 테스트:
```bash
curl http://127.0.0.1:8787/api/v1/platforms
```
예상: `{"code":0,"data":[{"code":"juliang","name":"巨量引擎","capabilities":["report","campaign","creative","oauth"]}]}`

- [x] **Step 4: 관리 백엔드 시작 및 연동 검증**
...
3. 대시보드 확인(빈 데이터)
4. 계정 관리 → 계정 바인딩(巨量引擎 공식 AppId 필요)
5. 테스트 광고 계획 생성
...
Phase 1 완료 후 다음 상태에 도달해야 합니다:

1. ✅ webman v2 서버 시작 가능, MySQL, Redis 연결 정상
2. ✅ 멀티 테넌트 테이블 `tenants` 구축, TenantIdentify 미들웨어 사용 가능
3. ✅ 플랫폼 어댑터 인터페이스 `PlatformAdapter` 정의 완료, 巨量引擎 어댑터가 전 메서드 구현
4. ✅ OAuth 인증 흐름 완전: 인증 URL 획득 → 콜백 처리 → Token 저장 → 계정 바인딩
5. ✅ 통일 집행 테이블 `campaigns/ad_groups/creatives`와 보고서 테이블 `report_metrics/report_extras` 구축
6. ✅ RESTful API 인증 접근 가능: `POST /auth/login` → Token 획득 → 다른 API 호출
7. ✅ 관리 백엔드 로그인 가능, 대시보드/계정 관리/계획 관리 페이지 사용 가능
8. ✅ 관리 백엔드에서 광고 계획 생성 후 巨量引擎 플랫폼에 동기화
