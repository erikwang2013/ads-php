# Phase 1: План реализации базового каркаса
...
**Goal:** построить каркас сервера webman v2 + админ-панели webman-admin v2, реализовать мультитенантность, аутентификацию и права, управление аккаунтами, фреймворк OAuth-процесса и на примере Ocean Engine пройти полный цикл.

**Architecture:** плагинная архитектура webman v2, в service/plugin/ по обязанностям разделены четыре плагина ads-tenant, ads-account, ads-platform, ads-api; админ-панель на Vue3+TS+Element Plus, состояние через Pinia, Axios для интеграции с бэкенд API.
...
### Task 1: Инициализация серверного проекта webman v2
...
- [x] **Step 1: Создать composer.json и установить зависимости**
...
- [x] **Step 2: Создать конфигурацию окружения .env**
...
- [x] **Step 3: Создать config/app.php**
...
- [x] **Step 4: Создать config/database.php с поддержкой динамических источников данных**
...
- [x] **Step 5: Создать config/redis.php**
...
- [x] **Step 6: Создать config/middleware.php**
...
- [x] **Step 7: Создать support/Response.php — единый ответ**
...
- [x] **Step 8: Создать service/start.php**
...
- [x] **Step 9: Проверить запуск проекта**

Run: `cd service && php start.php start`
Expected: webman v2 успешно запущен, слушает порт 8787
...
### Task 2: Создание плагина мультитенантности ads-tenant
...
- [x] **Step 2: Создать database.php — маршрутизацию источников данных**
...
- [x] **Step 3: Создать модель Tenant**
...
- [x] **Step 4: Создать middleware TenantIdentify**
...
- [x] **Step 5: Создать миграционный SQL**
...
### Task 3: Создание ядра адаптеров ads-platform
...
- [x] **Step 1: Создать интерфейс PlatformAdapter**
...
- [x] **Step 3: Создать ReportRequest**
...
- [x] **Step 4: Создать движок сопоставления полей FieldMapping**
...
- [x] **Step 5: Создать AdapterRegistry**
...
- [x] **Step 6: Создать адаптер Ocean Engine (сопоставление полей + каркас интерфейса)**
...
- [x] **Step 8: Создать миграционный SQL (campaigns, ad_groups, creatives, report_metrics, report_extras)**
...
- [x] **Step 9: Зарегистрировать адаптер Ocean Engine в AdapterRegistry**
...
### Task 4: Создание плагина управления аккаунтами ads-account
...
- [x] **Step 2: Создать модель PlatformAccount**
...
- [x] **Step 3: Создать модель AuthToken (для хранения state в OAuth-процессе)**
...
### Task 5: Создание плагина ads-api (RESTful API + JWT-аутентификация)
...
- [x] **Step 2: Создать JWT AuthMiddleware**
...
- [x] **Step 3: Создать AuthController**
...
- [x] **Step 4: Создать PlatformController**
...
- [x] **Step 5: Создать AccountController**
...
- [x] **Step 6: Создать CampaignController**
...
- [x] **Step 7: Создать DashboardController**
...
- [x] **Step 8: Создать конфигурацию маршрутов route.php**
...
### Task 6: Инициализация проекта админ-панели webman-admin v2
...
- [x] **Step 2: Создать vite.config.ts**
...
- [x] **Step 7: Создать src/router/index.ts**
...
- [x] **Step 8: Создать src/api/index.ts (обёртка Axios)**
...
- [x] **Step 9: Установить зависимости и проверить**

Run: `cd admin && npm install && npm run dev`
Expected: dev-сервер Vite запущен на порту 5173
...
### Task 7: Макет админ-панели и страница входа
...
- [x] **Step 3: Создать LoginPage.vue**
...
- [x] **Step 6: Создать AppLayout.vue**
...
### Task 8: Ключевые страницы админ-панели
...
- [x] **Step 1: Создать служебные функции format.ts**
...
- [x] **Step 2: Создать MetricCard.vue**
...
- [x] **Step 3: Создать PlatformBadge.vue**
...
- [x] **Step 4: Создать DashboardPage.vue**
...
- [x] **Step 5: Создать AccountList.vue**
...
- [x] **Step 6: Создать AccountBind.vue**
...
- [x] **Step 7: Создать CampaignList.vue**
...
- [x] **Step 8: Создать файлы обёрток API**
...
- [x] **Step 9: Проверить функциональность админ-панели**

Run: `cd admin && npm run dev`
Config: убедиться, что vite proxy указывает на сервер webman, порт 8787
Check: вход → дашборд → управление аккаунтами → создание плана → запуск/остановка плана
...
### Task 9: Загрузочная конфигурация и проверка запуска проекта

**Files:**
- Create: `service/start.php` (изменить, если существует)
- Create: `README.md`

- [x] **Step 1: Добавить в service/start.php загрузку плагинов и инициализацию БД**
...
- [x] **Step 2: Выполнить миграции базы данных**
...
- [x] **Step 3: Запустить сервер и проверить API**
...
- [x] **Step 4: Запустить админ-панель и проверить совместную работу**
...
3. Посмотреть дашборд (пустые данные)
4. Перейти в управление аккаунтами → привязать аккаунт (нужен официальный AppId Ocean Engine)
5. Создать тестовый рекламный план

- [x] **Step 5: Закоммитить финальное состояние**
...
После завершения Phase 1 должно быть достигнуто следующее состояние:

1. ✅ Сервер webman v2 запускается, подключение MySQL и Redis в норме
2. ✅ Таблица мультитенантности `tenants` создана, middleware TenantIdentify работает
3. ✅ Интерфейс адаптера платформы `PlatformAdapter` определён полностью, адаптер Ocean Engine реализует все методы
4. ✅ OAuth-процесс полный: получение URL авторизации → обработка колбэка → хранение Token → привязка аккаунта
5. ✅ Единые таблицы размещения `campaigns/ad_groups/creatives` и таблицы отчётов `report_metrics/report_extras` созданы
6. ✅ RESTful API доступен с аутентификацией: `POST /auth/login` → получение Token → вызов остальных интерфейсов
7. ✅ Админ-панель позволяет войти, страницы дашборда/управления аккаунтами/управления планами работают
