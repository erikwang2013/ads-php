# Phase 2: План реализации расширения рекламных платформ + движка отчётов
...
**Goal:** добавить адаптеры Baidu Marketing и Taobao Ads, реализовать реальное планирование синхронизации данных, усилить движок отчётов поддержкой пользовательских многомерных запросов.

**Architecture:** каждая рекламная платформа реализует интерфейс PlatformAdapter и единообразно регистрируется через AdapterRegistry. Синхронизация данных использует планировщик webman/crontab; в процессе синхронизации нативные данные платформ через FieldMapping преобразуются в единый формат и записываются в таблицу report_metrics. Движок отчётов на основе существующего DashboardController дополняется возможностью пользовательских запросов в CampaignController.

**Tech Stack:** PHP 8.2+, webman v2, webman/crontab, curl (API calls), Vue 3, ECharts 5

**Основа Phase 1:**
- Интерфейс PlatformAdapter, AdapterRegistry, FieldMapping — всё готово
- Адаптер Ocean Engine реализован по полному циклу (OAuth → синхронизация → операции размещения)
- Таблица report_metrics создана, DashboardController::summary() умеет агрегировать по дате/платформе
- Таблица campaigns создана, CampaignController с полным CRUD
- OAuthService — общий процесс авторизации готов

---

## Структура файлов (новое/изменённое в Phase 2)
...
### Task 10: Создание адаптера Baidu Marketing

**Files:**
- Create: `service/plugin/ads-platform/adapter/Baidu.php`
- Modify: `service/plugin/ads-platform/config/bootstrap.php`

Основная интеграция API Baidu Marketing:
- Адрес OAuth-авторизации: `https://u.baidu.com/oauth/authorize`
- Обмен Token: `https://u.baidu.com/oauth/token`
- Запрос рекламных планов: `/json/sms/service/CampaignService/getCampaign`
- Запрос отчётов: `/json/sms/service/ReportService/getProfessionalReportId` + `getReportData`

#### Step 1: Создать адаптер Baidu.php
...
#### Step 2: Зарегистрировать адаптер Baidu
...
### Task 11: Создание адаптера Taobao Ads

**Files:**
- Create: `service/plugin/ads-platform/adapter/Taobao.php`
- Modify: `service/plugin/ads-platform/config/bootstrap.php`

Основная интеграция API Taobao Ads (Alimama):
- OAuth-авторизация: OAuth 2.0 открытой платформы Taobao
- API endpoint: `https://api.taobao.com/router/rest`
- Рекламные планы: `alimama.campaign.get`
- Отчёты: `alimama.report.get`

#### Step 1: Создать Taobao.php
...
#### Step 2: Зарегистрировать адаптер Taobao
...
### Task 12: Создание задач синхронизации данных + усиление движка отчётов
...
#### Step 1: Создать плагин ads-task
...
#### Step 2: Создать TokenRefreshTask
...
#### Step 4: Создать конфигурацию cron.php для задач по расписанию
...
#### Step 5: Создать плагин ads-report + ReportBuilder
...
#### Step 6: Создать ReportController
...
#### Step 7: Добавить маршрут отчётов

В аутентифицированную группу в `route.php` добавить:
```php
...
1. ✅ Адаптер Baidu Marketing реализует все методы PlatformAdapter
2. ✅ Адаптер Taobao Ads реализует все методы PlatformAdapter (включая алгоритм подписи Taobao)
3. ✅ Обработка OAuth-колбэков единообразно распределяется через AdapterRegistry
4. ✅ Адаптеры Baidu/Taobao зарегистрированы в bootstrap.php
5. ✅ TokenRefreshTask каждые 55 минут сканирует истёкшие Token и обновляет их
6. ✅ DataSyncTask каждые 10 минут забирает данные размещения и отчёты всех платформ
7. ✅ API пользовательских отчётов поддерживает комбинации нескольких измерений (platform/date/campaign) и нескольких метрик
8. ✅ Синхронизированные данные корректно записываются в таблицу report_metrics
