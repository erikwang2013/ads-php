# Phase 2: 广告平台扩展 + 报表引擎 कार्यान्वयन योजना
...
> **एजेंटिक वर्कर्स के लिए:** अनिवार्य उप-कौशल: इस योजना को कार्य-दर-कार्य लागू करने के लिए superpowers:subagent-driven-development का उपयोग करें।

**लक्ष्य:** बैदू मार्केटिंग और ताओबाओ एडवरटाइज़िंग एडाप्टर जोड़ें, वास्तविक डेटा सिंक शेड्यूलिंग लागू करें, और कस्टम मल्टी-डाइमेंशन क्वेरी का समर्थन करने के लिए रिपोर्ट इंजन को बेहतर बनाएँ।

**आर्किटेक्चर:** हर एडवरटाइज़िंग प्लेटफ़ॉर्म PlatformAdapter इंटरफ़ेस लागू करता है, जिसे AdapterRegistry के माध्यम से एकीकृत रूप से पंजीकृत किया जाता है। डेटा सिंक webman/crontab निर्धारित शेड्यूलिंग का उपयोग करता है; सिंक के दौरान प्लेटफ़ॉर्म के मूल डेटा को FieldMapping के माध्यम से एकीकृत प्रारूप में बदलकर report_metrics तालिका में लिखा जाता है। रिपोर्ट इंजन मौजूदा DashboardController के आधार पर CampaignController में कस्टम क्वेरी क्षमता जोड़ता है।

**तकनीकी स्टैक:** PHP 8.2+, webman v2, webman/crontab, curl (API कॉल), Vue 3, ECharts 5

**Phase 1 मौजूदा आधार:**
- PlatformAdapter इंटरफ़ेस, AdapterRegistry, FieldMapping सभी तैयार हैं
- जुलियांग इंजन एडाप्टर ने पूरी श्रृंखला लागू की है (OAuth → सिंक → डिलीवरी ऑपरेशन)
- report_metrics तालिका बन चुकी है, DashboardController::summary() में दिनांक/प्लेटफ़ॉर्म द्वारा एग्रीगेट करने की क्षमता है
- campaigns तालिका बन चुकी है, CampaignController में पूर्ण CRUD है
- OAuthService सामान्य प्राधिकरण प्रवाह तैयार है

---

## फ़ाइल संरचना (Phase 2 नई/संशोधित)
...
### Task 10: बैदू मार्केटिंग एडाप्टर बनाएँ

**फ़ाइलें:**
- Create: `service/plugin/ads-platform/adapter/Baidu.php`
- Modify: `service/plugin/ads-platform/config/bootstrap.php`

बैदू मार्केटिंग API मुख्य इंटीग्रेशन:
- OAuth प्राधिकरण पता: `https://u.baidu.com/oauth/authorize`
- Token विनिमय: `https://u.baidu.com/oauth/token`
- एडवरटाइज़िंग प्लान क्वेरी: `/json/sms/service/CampaignService/getCampaign`
- रिपोर्ट क्वेरी: `/json/sms/service/ReportService/getProfessionalReportId` + `getReportData`

#### Step 1: Baidu.php एडाप्टर बनाएँ
...
#### Step 2: बैदू एडाप्टर पंजीकृत करें
...
### Task 11: ताओबाओ एडवरटाइज़िंग एडाप्टर बनाएँ

**फ़ाइलें:**
- Create: `service/plugin/ads-platform/adapter/Taobao.php`
- Modify: `service/plugin/ads-platform/config/bootstrap.php`

ताओबाओ एडवरटाइज़िंग (अलीमामा) API मुख्य इंटीग्रेशन:
- OAuth प्राधिकरण: ताओबाओ ओपन प्लेटफ़ॉर्म OAuth 2.0
- API एंडपॉइंट: `https://api.taobao.com/router/rest`
- एडवरटाइज़िंग प्लान: `alimama.campaign.get`
- रिपोर्ट: `alimama.report.get`

#### Step 1: Taobao.php बनाएँ
...
#### Step 2: ताओबाओ एडाप्टर पंजीकृत करें
...
### Task 12: डेटा सिंक टास्क + रिपोर्ट इंजन सुधार बनाएँ
...
#### Step 4: cron.php निर्धारित कार्य कॉन्फ़िगरेशन बनाएँ
...
#### Step 5: ads-report प्लगइन + ReportBuilder बनाएँ
...
#### Step 7: रिपोर्ट रूट जोड़ें

`route.php` में प्रमाणीकरण समूह में जोड़ें:
```php
...
1. ✅ बैदू मार्केटिंग एडाप्टर सभी PlatformAdapter विधियाँ लागू करता है
2. ✅ ताओबाओ एडवरटाइज़िंग एडाप्टर सभी PlatformAdapter विधियाँ लागू करता है (ताओबाओ सिग्नेचर एल्गोरिदम सहित)
3. ✅ OAuth कॉलबैक प्रोसेसिंग AdapterRegistry के माध्यम से एकीकृत रूप से विभाजित होती है
4. ✅ बैदू/ताओबाओ एडाप्टर bootstrap.php में पंजीकृत हैं
5. ✅ TokenRefreshTask हर 55 मिनट में समाप्त Token स्कैन करके रीफ़्रेश करता है
6. ✅ DataSyncTask हर 10 मिनट में सभी प्लेटफ़ॉर्म के डिलीवरी डेटा और रिपोर्ट खींचता है
7. ✅ रिपोर्ट कस्टम क्वेरी API मल्टी-डाइमेंशन (platform/date/campaign) और मल्टी-मेट्रिक संयोजन का समर्थन करती है
8. ✅ सिंक किया गया डेटा सही ढंग से report_metrics तालिका में लिखा जाता है
