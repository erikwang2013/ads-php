# Phase 1 : Squelette de base — Plan d'implémentation
...
**Objectif :** Mettre en place le squelette du serveur webman v2 + du panneau d'administration webman-admin v2, implémenter le multi-tenant, l'authentification et les permissions, la gestion des comptes, le cadre du flux OAuth, et parcourir toute la chaîne avec 巨量引擎 (Ocean Engine) comme première plateforme.

**Architecture :** Architecture à plugins webman v2 ; sous service/plugin/, quatre plugins découpés par responsabilité : ads-tenant, ads-account, ads-platform, ads-api ; le panneau d'administration utilise Vue3+TS+Element Plus, avec Pinia pour la gestion d'état et Axios pour les API backend.
...
### Task 1: Initialiser le projet serveur webman v2
...
- [x] **Step 1: Créer composer.json et installer les dépendances**
...
- [x] **Step 2: Créer la configuration d'environnement .env**
...
- [x] **Step 4: Créer config/database.php avec prise en charge des sources de données dynamiques**
...
- [x] **Step 5: Créer config/redis.php**
...
- [x] **Step 6: Créer config/middleware.php**
...
- [x] **Step 7: Créer support/Response.php pour la réponse unifiée**
...
- [x] **Step 8: Créer service/start.php**
...
- [x] **Step 9: Vérifier le démarrage du projet**

Run: `cd service && php start.php start`
Attendu : webman v2 démarre avec succès, écoute sur le port 8787
...
### Task 2: Créer le plugin multi-tenant ads-tenant
...
- [x] **Step 2: Créer le routage de source de données database.php**
...
- [x] **Step 3: Créer le modèle Tenant**
...
- [x] **Step 4: Créer le middleware TenantIdentify**
...
- [x] **Step 5: Créer la migration SQL**
...
### Task 3: Créer le cœur des adaptateurs ads-platform
...
- [x] **Step 1: Créer l'interface PlatformAdapter**
...
- [x] **Step 4: Créer le moteur de mapping de champs FieldMapping**
...
- [x] **Step 5: Créer AdapterRegistry**
...
- [x] **Step 6: Créer l'adaptateur 巨量引擎 (mapping de champs + squelette d'interface)**
...
- [x] **Step 8: Créer la migration SQL (campaigns, ad_groups, creatives, report_metrics, report_extras)**
...
- [x] **Step 9: Enregistrer l'adaptateur 巨量引擎 dans AdapterRegistry**
...
### Task 4: Créer le plugin de gestion des comptes ads-account
...
- [x] **Step 2: Créer le modèle PlatformAccount**
...
- [x] **Step 3: Créer le modèle AuthToken (pour le stockage du state dans le flux OAuth)**
...
### Task 5: Créer le plugin ads-api (API RESTful + authentification JWT)
...
- [x] **Step 2: Créer le middleware JWT AuthMiddleware**
...
- [x] **Step 4: Créer PlatformController**
...
- [x] **Step 5: Créer AccountController**
...
- [x] **Step 6: Créer CampaignController**
...
- [x] **Step 7: Créer DashboardController**
...
- [x] **Step 8: Créer la configuration des routes route.php**
...
### Task 6: Initialiser le projet de panneau d'administration webman-admin v2
...
- [x] **Step 7: Créer src/router/index.ts**
...
- [x] **Step 8: Créer src/api/index.ts (encapsulation Axios)**
...
- [x] **Step 9: Installer les dépendances et vérifier**

Run: `cd admin && npm install && npm run dev`
Attendu : le serveur de développement Vite démarre sur le port 5173
...
### Task 7: Disposition du panneau d'administration & page de connexion
...
### Task 8: Pages principales du panneau d'administration
...
- [x] **Step 1: Créer les fonctions utilitaires format.ts**
...
- [x] **Step 3: Créer PlatformBadge.vue**
...
- [x] **Step 4: Créer DashboardPage.vue**
...
- [x] **Step 5: Créer AccountList.vue**
...
- [x] **Step 6: Créer AccountBind.vue**
...
- [x] **Step 7: Créer CampaignList.vue**
...
- [x] **Step 8: Créer les fichiers d'encapsulation API**
...
- [x] **Step 9: Vérifier les fonctionnalités du panneau d'administration**

Run: `cd admin && npm run dev`
Config : s'assurer que le proxy vite pointe vers le serveur webman sur le port 8787
Check : connexion → tableau de bord → gestion des comptes → création de plan → démarrage/arrêt de plan
...
### Task 9: Guide de configuration & validation du démarrage du projet

**Files:**
- Create: `service/start.php` (modifier s'il existe)
- Create: `README.md`

- [x] **Step 1: Ajouter le chargement des plugins et l'initialisation de la base de données dans service/start.php**
...
- [x] **Step 2: Exécuter les migrations de base de données**
...
- [x] **Step 3: Démarrer le serveur et vérifier l'API**
...
```
Attendu : `{"code":0,"data":{"access_token":"...","user":{...}}}`

Test des plateformes :
```bash
curl http://127.0.0.1:8787/api/v1/platforms
```
Attendu : `{"code":0,"data":[{"code":"juliang","name":"巨量引擎","capabilities":["report","campaign","creative","oauth"]}]}`

- [x] **Step 4: Démarrer le panneau d'administration et vérifier l'interconnexion**
...
3. Consulter le tableau de bord (données vides)
4. Aller dans la gestion des comptes → lier un compte (nécessite un AppId officiel 巨量引擎)
5. Créer un plan publicitaire de test

- [x] **Step 5: Committer l'état final**
...
Une fois la Phase 1 terminée, l'état suivant doit être atteint :

1. ✅ Le serveur webman v2 démarre, connexions MySQL et Redis normales
2. ✅ La table multi-tenant `tenants` est créée, le middleware TenantIdentify fonctionne
3. ✅ L'interface d'adaptateur `PlatformAdapter` est complètement définie, l'adaptateur 巨量引擎 implémente toutes les méthodes
4. ✅ Le flux d'autorisation OAuth est complet : obtention de l'URL d'autorisation → traitement du callback → stockage du Token → liaison du compte
5. ✅ Les tables de diffusion unifiées `campaigns/ad_groups/creatives` et les tables de rapports `report_metrics/report_extras` sont créées
6. ✅ L'API RESTful est accessible avec authentification : `POST /auth/login` → obtention du Token → appel des autres interfaces
7. ✅ Le panneau d'administration permet la connexion, les pages tableau de bord/gestion des comptes/gestion des plans fonctionnent
8. ✅ Le panneau d'administration permet de créer un plan publicitaire et de le synchroniser vers la plateforme 巨量引擎
