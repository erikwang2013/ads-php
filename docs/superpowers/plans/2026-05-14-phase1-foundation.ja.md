# Phase 1: 基礎骨架 Implementation Plan
...
**Goal:** webman v2 サーバーサイド + webman-admin v2 管理バックエンドの骨組みを構築し、マルチテナント、認証権限、アカウント管理、OAuth フローのフレームワークを実装し、巨量引擎を最初のプラットフォームとして全チェーンを一通り動かします。

**Architecture:** webman v2 のプラグイン化アーキテクチャ。service/plugin/ 配下に責務ごとに ads-tenant、ads-account、ads-platform、ads-api の 4 プラグインを分割。管理バックエンドは Vue3+TS+Element Plus を使用し、Pinia で状態管理、Axios でバックエンド API に接続します。
...
### Task 1: webman v2 サーバーサイドプロジェクトの初期化
...
- [x] **Step 1: composer.json を作成し依存をインストール**
...
- [x] **Step 4: config/database.php を作成（動的データソース対応）**
...
- [x] **Step 6: config/middleware.php を作成**
...
- [x] **Step 7: support/Response.php 統一レスポンスを作成**
...
- [x] **Step 8: service/start.php を作成**
...
- [x] **Step 9: プロジェクトの起動を検証**

Run: `cd service && php start.php start`
Expected: webman v2 が起動成功、8787 ポートで待ち受け
...
### Task 2: ads-tenant マルチテナントプラグインの作成
...
- [x] **Step 2: database.php データソースルーティングを作成**
...
- [x] **Step 4: TenantIdentify ミドルウェアを作成**
...
### Task 3: ads-platform アダプターコアの作成
...
- [x] **Step 1: PlatformAdapter インターフェースを作成**
...
- [x] **Step 4: FieldMapping フィールドマッピングエンジンを作成**
...
- [x] **Step 6: 巨量引擎アダプターを作成（フィールドマッピング + インターフェース骨組み）**
...
- [x] **Step 8: マイグレーション SQL を作成（campaigns, ad_groups, creatives, report_metrics, report_extras）**
...
- [x] **Step 9: 巨量引擎アダプターを AdapterRegistry に登録**

`service/plugin/ads-platform/config/bootstrap.php` を作成:
...
### Task 4: ads-account アカウント管理プラグインの作成
...
- [x] **Step 2: PlatformAccount モデルを作成**
...
- [x] **Step 3: AuthToken モデルを作成（OAuth フローの state 保存用）**
...
### Task 5: ads-api プラグインの作成（RESTful API + JWT 認証）
...
- [x] **Step 2: JWT AuthMiddleware を作成**
...
- [x] **Step 4: PlatformController を作成**
...
- [x] **Step 5: AccountController を作成**
...
- [x] **Step 6: CampaignController を作成**
...
- [x] **Step 7: DashboardController を作成**
...
### Task 6: webman-admin v2 管理バックエンドプロジェクトの初期化
...
- [x] **Step 7: src/router/index.ts を作成**
...
- [x] **Step 8: src/api/index.ts を作成（Axios ラッパー）**
...
- [x] **Step 9: 依存をインストールして検証**

Run: `cd admin && npm install && npm run dev`
Expected: Vite 開発サーバーが 5173 ポートで起動
...
- [x] **Step 1: ユーティリティ関数 format.ts を作成**
...
- [x] **Step 3: PlatformBadge.vue を作成**
...
- [x] **Step 4: DashboardPage.vue を作成**
...
- [x] **Step 9: 管理バックエンド機能を検証**

Run: `cd admin && npm run dev`
Config: vite proxy が webman サーバー 8787 ポートを指すことを確認
Check: ログイン → ダッシュボード → アカウント管理 → 計画作成 → 計画の起動/停止
...
### Task 9: 設定ガイド & プロジェクト起動検証

**Files:**
- Create: `service/start.php` (あれば変更)
- Create: `README.md`

- [x] **Step 1: service/start.php にプラグイン読み込みとデータベース初期化を追加**
...
3. ダッシュボードを確認（空データ）
4. アカウント管理 → アカウントを紐付け（巨量引擎の正式な AppId が必要）
5. テスト用の広告計画を作成
...
Phase 1 完了後、以下の状態に達すること:

1. ✅ webman v2 サーバーが起動可能、MySQL、Redis 接続が正常
2. ✅ マルチテナントテーブル `tenants` 作成済み、TenantIdentify ミドルウェアが利用可能
3. ✅ プラットフォームアダプターインターフェース `PlatformAdapter` の定義が完全、巨量引擎アダプターが全メソッドを実装
4. ✅ OAuth 認可フローが完全: 認可 URL 取得 → コールバック処理 → Token 保存 → アカウント紐付け
5. ✅ 統一配信テーブル `campaigns/ad_groups/creatives` とレポートテーブル `report_metrics/report_extras` 作成済み
6. ✅ RESTful API が認証付きでアクセス可能: `POST /auth/login` → Token 取得 → 他 API を呼び出し
7. ✅ 管理バックエンドにログイン可能、ダッシュボード/アカウント管理/計画管理ページが利用可能
8. ✅ 管理バックエンドから広告計画を作成し、巨量引擎プラットフォームに同期可能
