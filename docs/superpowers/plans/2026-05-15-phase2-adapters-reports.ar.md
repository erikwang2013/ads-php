# المرحلة 2: خطة تنفيذ توسيع منصات الإعلانات + محرك التقارير
...
**الهدف:** إضافة محولي Baidu Marketing وTaobao Ads، وتنفيذ جدولة مزامنة بيانات حقيقية، وتعزيز محرك التقارير لدعم الاستعلامات المخصصة متعددة الأبعاد.

**البنية:** ينفّذ كل منصة إعلانية واجهة PlatformAdapter، مع التسجيل الموحد عبر AdapterRegistry. تُجدول مزامنة البيانات عبر webman/crontab، وأثناء المزامنة تُحوَّل البيانات الأصلية للمنصة عبر FieldMapping إلى التنسيق الموحد وتُكتب في جدول report_metrics. يضيف محرك التقارير، استنادًا إلى DashboardController الحالي، قدرة الاستعلام المخصص في CampaignController.

**Tech Stack:** PHP 8.2+, webman v2, webman/crontab, curl (استدعاءات API), Vue 3, ECharts 5

**أساسيات المرحلة 1 الموجودة:**
- واجهة PlatformAdapter وAdapterRegistry وFieldMapping جاهزة بالكامل
- محول Juliang منفذ بالسلسلة الكاملة (OAuth → مزامنة → عمليات الإطلاق)
- جدول report_metrics منشأ، وDashboardController::summary() لديه قدرة التجميع حسب التاريخ/المنصة
- جدول campaigns منشأ، وCampaignController لديه CRUD كامل
- تدفق التفويض العام OAuthService جاهز

---

## هيكل الملفات (إضافات/تعديلات المرحلة 2)
...
### المهمة 10: إنشاء محول Baidu Marketing

**Files:**
- Create: `service/plugin/ads-platform/adapter/Baidu.php`
- Modify: `service/plugin/ads-platform/config/bootstrap.php`

الربط الأساسي مع API Baidu Marketing:
- عنوان تفويض OAuth: `https://u.baidu.com/oauth/authorize`
- تبادل Token: `https://u.baidu.com/oauth/token`
- الاستعلام عن خطط الإعلان: `/json/sms/service/CampaignService/getCampaign`
- الاستعلام عن التقارير: `/json/sms/service/ReportService/getProfessionalReportId` + `getReportData`

#### الخطوة 1: إنشاء محول Baidu.php
...
### المهمة 11: إنشاء محول Taobao Ads

**Files:**
- Create: `service/plugin/ads-platform/adapter/Taobao.php`
- Modify: `service/plugin/ads-platform/config/bootstrap.php`

الربط الأساسي مع API إعلانات Taobao (Alimama):
- تفويض OAuth: OAuth 2.0 لمنصة Taobao المفتوحة
- نقطة نهاية API: `https://api.taobao.com/router/rest`
- خطط الإعلان: `alimama.campaign.get`
- التقارير: `alimama.report.get`

#### الخطوة 1: إنشاء Taobao.php
...
### المهمة 12: إنشاء مهام مزامنة البيانات + تعزيز محرك التقارير
...
#### الخطوة 2: إنشاء TokenRefreshTask
...
#### الخطوة 4: إنشاء تكوين المهام المجدولة cron.php
...
#### الخطوة 5: إنشاء إضافة ads-report + ReportBuilder
...
#### الخطوة 6: إنشاء ReportController
...
#### الخطوة 7: إضافة مسارات التقارير

أضِف في مجموعة المصادقة في `route.php`:
```php
...
1. ✅ محول Baidu Marketing ينفّذ جميع طرق PlatformAdapter
2. ✅ محول Taobao Ads ينفّذ جميع طرق PlatformAdapter (بما في ذلك خوارزمية توقيع Taobao)
3. ✅ معالجة استدعاءات OAuth موحدة التوجيه عبر AdapterRegistry
4. ✅ محولا Baidu/Taobao مسجلان في bootstrap.php
5. ✅ TokenRefreshTask يفحص Token المنتهي ويجدده كل 55 دقيقة
6. ✅ DataSyncTask يجلب بيانات الإطلاق والتقارير لكل منصة كل 10 دقائق
7. ✅ API الاستعلام المخصص للتقارير يدعم تركيبات متعددة الأبعاد (platform/date/campaign) ومتعددة المقاييس
8. ✅ بيانات المزامنة تُكتب بشكل صحيح في جدول report_metrics
