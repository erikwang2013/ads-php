...
| RetrySyncTask | Every 3 min | Retry failed syncs (ads_sync_errors table, max 3 retries, exponential backoff) |
...
| Phase 10 | Chinese comments in config files + .env comments + platform credentials docs + `ads_` table prefix rewrite + BIGINT PK | ✅ |
