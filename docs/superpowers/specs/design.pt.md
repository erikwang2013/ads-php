...
| RetrySyncTask | A cada 3 minutos | Repete sincronizações com falha (tabela ads_sync_errors, máximo de 3 vezes, backoff exponencial) |
...
| Phase 10 | Comentários em chinês nos arquivos de configuração + comentários no .env + documentação de credenciais das plataformas + reescrita do prefixo ads_ nas tabelas + BIGINT PK | ✅ |
