...
Verwendung der 7 Erik Stack-Pakete in diesem Projekt.

## Pakete und Verwendung

### snowflake-php — Verteilte ID-Generierung
```php
...
### hashids — API-ID-Verschlüsselung
```php
...
### encryption — Verschlüsselung auf API-Ebene
Über die globale Middleware aktiviert. Clients senden den Header `X-Encrypted: 1`.
Der Request-Body wird automatisch entschlüsselt, der Response-Body automatisch verschlüsselt.
```env
APP_ENCRYPTION_KEY=your-32-char-key-here
```

### encryptable — Verschlüsselung von Datenbankfeldern
```php
...
### webman-scout — Elasticsearch-Synchronisierung
Konfiguriert in `service/config/scout.php`. Modelle mit dem `Searchable`-Trait synchronisieren automatisch zu ES.
```env
...
Verwendet in `PlatformBadge.vue` über Unicode-Flaggen-Emojis. Kein Laufzeit-Import erforderlich.
```typescript
