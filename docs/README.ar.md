...
<p align="center"><img src="docs/diagrams/svg/pet.svg" width="160" alt="تميمة المشروع 阿章"></p>

**阿章** (zhāng، «أخطبوط») هو تميمة هذا المشروع: أخطبوط. يمتد بأذرعه الثماني دفعة واحدة، وكل ذراع تتولى مهمتها وتمتد إلى 29 منصة في آن واحد — وهذا تحديدًا ما يفعله هذا النظام.

| الشكل | الدلالة | التنفيذ المقابل |
|------|------|---------|
| ثمانية أذرع | ثماني إضافات، كل واحدة تتولى مهمتها، تعمل بالتوازي | ads-account / ads-alert / ads-api / ads-platform / ads-report / ads-storage / ads-task / ads-tenant |
| ممصات الأذرع | البيانات تُجذب وتتجمّع على طول الطريق | 29 منصة → واجهة موحدة `/api/v1` |
| عينان تنظران إلى الأمام | مراقبة عدة أطراف في آن واحد | Vue Admin / Flutter / HarmonyOS ثلاثة أطراف تتشارك مجموعة API واحدة |
| رمش + صعود وهبوط | استقصاء دوري وحراسة صامتة | 6 مهام مجدولة (3/5/10/10/15/55 دقيقة) تجمع البيانات في دورة؛ 22 حماية تعمل بصمت |
| 29 عقدة على الحلقة الخارجية | 29 منصة متصلة على الدوام | 16 محلية + 13 دولية، وكل واحدة تحت المراقبة |

تم دمج 阿章 في الكود: **صفحة تسجيل الدخول** و**favicon** في لوحة الإدارة، و**صفحة توثيق API `/docs`** في طرف Service، وكذلك جميع صفحات الوثائق.

> الكود المصدري SVG [docs/diagrams/svg/pet.svg](docs/diagrams/svg/pet.svg) (بلا نص، نسخة واحدة مشتركة لوثائق 13 لغة؛ يتضمن حركة رمش وتنفس SMIL)

---

## نظرة عامة

**Ads Platform** هو نظام إدارة إعلانات متعدد المنصات يدمج **29 منصة إعلانية** (16 محلية + 13 دولية)، مع إدارة موحّدة لنشر الإعلانات وتقارير البيانات عبر المنصات.
...
- **تميمة المشروع** — الأخطبوط «阿章»، ثمانية أذرع تعمل بالتوازي، وحراسة على مدار 24 ساعة لـ 29 منصة ([pet.svg](docs/diagrams/svg/pet.svg))
...
|----|------|------|
| الخادم | webman v2 + PHP 8.4+ | 8 إضافات، 76 نقطة نهاية API |
...
# API: http://localhost/api/v1（الإصدار مُثبَّت في مسار URL، ولا يُمرَّر عبر Header）
```
...
│   │   └── img/pet.svg                # تميمة المشروع «阿章»، تُعرض في صفحة /docs
...
│   ├── public/web/
│   │   ├── public/pet.svg             # تميمة المشروع «阿章»، favicon + صفحة تسجيل الدخول
│   │   ├── src/
│   │   │   ├── views/                 # 21 صفحة Vue
