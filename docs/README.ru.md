...
<p align="center"><img src="docs/diagrams/svg/pet.svg" width="160" alt="Талисман проекта 阿章"></p>

**阿章** (zhāng, «осьминог») — талисман этого проекта: осьминог. Он действует всеми восемью щупальцами сразу, каждое занято своим делом и тянется к 29 платформам — ровно то, чем занимается эта система.

| Образ | Смысл | Реализация |
|------|------|---------|
| Восемь щупалец | Восемь плагинов, каждый занят своим делом, работают параллельно | ads-account / ads-alert / ads-api / ads-platform / ads-report / ads-storage / ads-task / ads-tenant |
| Присоски на щупальцах | Данные присасываются и собираются по пути | 29 платформ → единый `/api/v1` |
...
| Моргание + покачивание | Периодический опрос и тихая охрана | 6 плановых задач (3/5/10/10/15/55 минут) циклического сбора; 22 меры защиты работают молча |
| 29 узлов на внешнем кольце | 29 платформ постоянно на связи | 16 китайских + 13 международных, за каждой следят |

阿章 уже стал частью кода: **страница входа** и **favicon** админ-панели, **страница API-документации `/docs`** на стороне Service, а также все страницы документации.

> Исходник SVG [docs/diagrams/svg/pet.svg](docs/diagrams/svg/pet.svg) (без текста, один общий файл для документации на 13 языках; содержит SMIL-анимацию моргания и дыхания)
...
- **Талисман проекта** — осьминог «阿章», восемь щупалец работают параллельно, круглосуточная вахта на 29 платформах ([pet.svg](docs/diagrams/svg/pet.svg))
...
|----|------|------|
| Сервер | webman v2 + PHP 8.4+ | 8 плагинов, 76 API-эндпоинтов |
...
│   │   └── img/pet.svg                # Талисман проекта «阿章», показывается на странице /docs
...
│   ├── public/web/
│   │   ├── public/pet.svg             # Талисман проекта «阿章», favicon + страница входа
│   │   ├── src/
