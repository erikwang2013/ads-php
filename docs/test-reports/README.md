...
| 套件 | 用例 | 断言 | 结果 | 报告 |
|------|------|------|------|------|
| PHP 单元测试（8 插件 + 中间件 + 熔断 + 存储） | 165 | — | ✅ 全部通过 | [php-unit.md](php-unit.md) |
| API 自动化测试（76 路由） | 123 | — | ✅ 全部通过 | [api.md](api.md) |
| UI 端到端（admin 18 页面） | 18 | — | ✅ 全部通过 | [ui-e2e.md](ui-e2e.md) |
| Go 模块 | — | — | N/A（无 Go 代码） | [go.md](go.md) |
| Rust 模块 | — | — | N/A（无 Rust 代码） | [rust.md](rust.md) |

**合计：288 PHP tests / 862 assertions + 18 UI E2E 用例，全部通过**

## SQL 合并结论

- 应用 SQL 仅有根目录 `install.sql`（29 张表），**无其他应用 SQL 文件**，无需合并。
...
**最终状态：288 tests / 862 assertions**（全量跑批存在顺序相关偶发失败，单测隔离运行全部通过，详见 [php-unit.md](php-unit.md)）。
