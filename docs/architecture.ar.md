...
![مخطط بنية النظام](diagrams/svg/architecture.ar.svg)
...
![مخطط تدفق الطلبات](diagrams/svg/request-flow.ar.svg)
...
![مخطط البنية الأمنية](diagrams/svg/security.ar.svg)
...
رقم إصدار API مثبت في مسار URL (`/api/v1/...`)، والمسارات مرتبطة ثابتًا بـ `plugin\ads_api\controller\v1\*`؛ ولا يُمرَّر في الترويسات. عند إضافة إصدار جديد، سجِّل مجموعة مسارات مستقلة (مثل `/api/v2` → `controller\v2\*`).
