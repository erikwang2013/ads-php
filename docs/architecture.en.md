...
The version number does not appear in the URL path. The version is passed via the `X-API-Version` header; `VersionMiddleware` reads it and sets `$request->apiVersion`. The `versioned()` helper replaces the version segment in the controller class with the request version at runtime.
