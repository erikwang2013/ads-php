...
flowchart TD
    ...
    Replay --> Version["7. VersionMiddleware<br/>X-API-Version 读取<br/>设置 request.apiVersion"]
    Version --> RateLimit["8. RateLimitMiddleware<br/>Redis 滑动窗口<br/>60次/60s 全局限流"]
    RateLimit --> LoginThrottle{"9. LoginThrottleMiddleware<br/>是否为登录路由?"}
    LoginThrottle -->|"是"| ThrottleCheck["5次失败 → 15分钟锁定<br/>Redis 计数 + TTL"]
    LoginThrottle -->|"否"| SessionLimit["10. SessionLimitMiddleware<br/>并发会话限制<br/>每用户最大 3 个活跃 Token"]
    ThrottleCheck --> SessionLimit
    SessionLimit --> SQLGuard["11. SqlGuardMiddleware<br/>SQL 注入模式检测<br/>UNION/DROP/ALTER/SLEEP"]
    SQLGuard --> Validation["12. ValidationMiddleware<br/>输入清洗<br/>trim + strip_tags"]
    Validation --> RespTime["13. ResponseTimeMiddleware<br/>记录 X-Response-Time<br/>慢请求日志 (>1s)"]
    RespTime --> Encryption{"14. EncryptionMiddleware<br/>X-Encrypted 头?"}
    Encryption -->|"是"| Decrypt["AES 解密请求 Body<br/>处理后 AES 加密响应"]
    Encryption -->|"否"| Auth["15. AuthMiddleware<br/>JWT Bearer Token 验证<br/>IP + User-Agent 绑定校验<br/>Token 黑名单检查"]
    Decrypt --> Auth
...
flowchart TD
    ...
    Client2 --> Version2["5. VersionMiddleware<br/>API 版本路由"]
    Version2 --> Rate2["6. RateLimitMiddleware<br/>滑动窗口限流"]
    Rate2 --> LoginT2{"7. LoginThrottleMiddleware<br/>登录路由?"}
    LoginT2 -->|"是"| LT2["5次失败 → 15分钟锁定"]
    LoginT2 -->|"否"| SQL2["8. SqlGuardMiddleware<br/>SQL 注入检测"]
    LT2 --> SQL2
    SQL2 --> Valid2["9. ValidationMiddleware<br/>输入清洗"]
    Valid2 --> CSRF2{"10. CsrfMiddleware<br/>非 GET/HEAD/OPTIONS?"}
...
```mermaid
flowchart LR
    Req["GET /api/campaigns<br/>Header: X-API-Version: v1"] --> VM["VersionMiddleware"]
    VM -->|"$request->apiVersion = 'v1'"| Route["versioned() 辅助函数"]
    Route -->|"运行时替换"| Ctrl["controller\\v1\\CampaignController::index()"]
