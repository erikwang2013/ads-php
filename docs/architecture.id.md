...
## 7. Mekanisme Route Versi API

Nomor versi tidak muncul di path URL. Versi dikirim melalui header `X-API-Version`, `VersionMiddleware` membacanya dan mengatur `$request->apiVersion`. Fungsi bantuan `versioned()` mengganti segmen versi di kelas controller dengan versi request saat runtime.
